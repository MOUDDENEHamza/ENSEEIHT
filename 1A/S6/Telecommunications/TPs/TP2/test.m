...
Symboles_1=2*bits-1; 
Symboles_2=2*bits-1;
Symboles = Symboles_1 + 1i*Symboles_2;
Suite_diracs_1=kron(Symboles_1, [1 zeros(1,Ns-1)]); 
Suite_diracs_2=kron(Symboles_2, [1 zeros(1,Ns-1)]); 
h = rcosdesign(alpha/2,span,Ns);
hr=h;
I = filter(h,1,[Suite_diracs_1 zeros(1,(span*Ns)/2)]);
Q = filter(h,1,[Suite_diracs2 zeros(1,(span*Ns)/2)]);
...
signal = I .* cos(2*pi*f*temps) - Q .* sin(2*pi*f*temps);
figure;
plot(signal);
title('signal émis');
xlabel('temps en s');
...
oeil = reshape(I,2*Ns,length(I)/(2*Ns));
figure;
plot(oeil);
title('oeil');
ylabel('x');
xlabel('temps en s');
signal_echant = s(1:Ns:end);

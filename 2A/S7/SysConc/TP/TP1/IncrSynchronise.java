...
public class IncrSynchronise {
    ...
    static void lancer(int nbA, Incrémenteur r) {
        ...
        for (int i = 0; i < nbA ; i++) {
            try {
                 ...
                 IncrSynchronise.activités[i] = new Thread(r,"t"+i);
            ...
            IncrSynchronise.activités[i].start();
                    }
    }

    static void finir() {
        // Attente de la terminaison des différents threads lancés
        for (int i = 0; i < IncrSynchronise.activités.length ; i++) {
            try {
                IncrSynchronise.activités[i].join();
            }
    ...
    public static void main(String[] args) {
        ...
        // Idée = retenir une moyenne des dernières mesures.
            ...
            for (long li = 0; li < nbActivités*IncrSynchronise.NB_IT; li++) {
                for (long j = 1; j<=IncrSynchronise.NB_IT_INTERNES; j++) {
                    IncrSynchronise.cpt=IncrSynchronise.cpt+j/j;
                    // j/j pour tenter de déjouer les optimisations du compilateur
                }
                try {
                    Thread.sleep(IncrSynchronise.Attente_ms, IncrSynchronise.Attente_nano);
            ...
            lancer(nbActivités, new IncrémenteurSyncInterne());
            finir();
            fin = System.nanoTime();
...
class IncrémenteurSyncExterne implements Incrémenteur {
    ...
    public void incr() {
        ...
        synchronized(IncrSynchronise.mutex) {
          for (long i = 0L; i < IncrSynchronise.NB_IT; i++) {
    ...
    public void run() {
        ...
        System.out.println("avant le thread " + Thread.currentThread().getName() + " : " +IncrSynchronise.cpt);
        this.incr();
        System.out.println("après le thread " + Thread.currentThread().getName() + " : "+ IncrSynchronise.cpt);
    }
}

class IncrémenteurSyncInterne implements Incrémenteur {
    /* Exemple d'incrémenteur :
     -la méthode incr effectue les incrémentations sans gérer les conflis d'accès au compteur
     - la méthode run() appelle incr()
     */
...
class IncrémenteurSyncPositionInter implements Incrémenteur {
    /* Exemple d'incrémenteur :
     -la méthode incr effectue les incrémentations sans gérer les conflis d'accès au compteur
     - la méthode run() appelle incr()
     */

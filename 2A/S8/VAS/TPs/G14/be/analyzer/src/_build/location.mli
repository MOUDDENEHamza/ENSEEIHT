...
module MapInd : Map.S with type key = (t*int)      (** Maps from locations and unroll id. *)
module Map : Map.S with type key = t      (** Maps from locations. *)
(* module MapUnroll : Map.S with type key = t*int  (\** Maps from locations and unroll index. *\) *)

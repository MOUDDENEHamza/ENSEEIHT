...
((isNan1 || isNan2), (isMInf1 || isMInf2), (isPInf1 || isPInf2), (Itv.sem_plus itv1 itv2))

let sem_minus x y =
match x, y with
(isNan1, isMInf1, isPInf1, itv1),(isNan2, isMInf2, isPInf2, itv2) -> 
(not(isNan1 && isNan2) || (isNan1&& not(isNan2)), 
  not(isMInf1 && isMInf2) || (isMInf1&& not(isMInf2)), 
  not(isPInf1 && isPInf2) || (isPInf1 &&not(isPInf2)), 
  (Itv.sem_minus itv1 itv2))

let sem_times x y = top (*TODO *)
let sem_div x y = top (*TODO *)

let sem_geq0 = function
  | t -> meet (false, false, true, Itv(0, None)) t

...
public class Block {
	...
	public static List<Instruction> fonctions = new ArrayList<>();
	...
	public boolean collect(HierarchicalScope<Declaration> _scope) {
		...
		for (Instruction inst : this.instructions) {
			_result = _result && inst.collectAndBackwardResolve(_scope);
		}
		return _result;
	}
	...
	public boolean resolve(HierarchicalScope<Declaration> _scope) {
		boolean _result = true;
		for (Instruction inst : this.instructions) {
			_result = _result && inst.fullResolve(_scope);
		}
		return _result;
	}
	...
	public boolean checkType() {
		boolean _result = true;
		for (Instruction inst : this.instructions) {
			_result = _result && inst.checkType();
		}
		return _result;
	}
	...
	public void allocateMemory(Register _register, int _offset) {
		...
		for (Instruction inst : this.instructions) {
			position += inst.allocateMemory(_register, position);
		}
		this.allocated = position - _offset;
	}
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		for (Instruction instruction : this.instructions) {
			if (instruction instanceof FunctionDeclaration) {
				this.fonctions.add(instruction);
			} else {
				fragment.append(instruction.getCode(_factory));
			}
		}
		//fragment.add(_factory.createPop(0, this.allocated));
		return fragment;
	}
